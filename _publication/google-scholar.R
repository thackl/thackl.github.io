...
#    author=str_replace_all(author, " (\\S) ", "\\1 "),
    ...
    map(function(x){
      x %>% glue_data('<p>{author} ({year}) <a href="https://scholar.google.com/scholar?oi=bibs&cluster={cid}&btnI=1&hl=en">{title}</a>, {journal}, {number}</p>') %>% str_replace_all("(, )+</p>", "</p>") %>%
  char2html()
    }) %>% rev
...
html_4 <- c(
  paste0('<p style="text-align: right; margin-top: -40px;"><small>Last updated <i>', format(Sys.Date(), format="%B %d, %Y"), '</i>. Pulled from my <a href="https://scholar.google.com/citations?hl=en&user=b8bWNkUAAAAJ">Google Scholar profile</a></small></p>'),
